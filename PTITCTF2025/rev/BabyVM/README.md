...
- Đề bài:
    ...
    ![alt text](../../images/babyvm01.png)

- Đây là một file thực thi elf 64-bit. Tuy nhiên, nó không thực sự là một chương trình viết bằng C/C++ bình thường.

    ![alt text](../../images/babyvm02.png)

- Ngay trên đề bài đã có hint nhỏ liên quan đến python, và khi thả file này vào IDA rồi xem bảng Strings cũng thấy các dấu hiệu liên quan đến python:

    ![alt text](../../images/babyvm03.png)

- Nhìn vào bảng Strings có thể thấy đây là 1 file cài đặt bằng python sử dụng pyinstaller. Vì thế mình đã sử dụng công cụ `pyinstxtractor` để extract và lấy được file [`chall.pyc`](chall_extracted/chall.pyc)
- Sau đó mình load file này lên công cụ pylingual

    ![alt text](../../images/babyvm04.png)

- Tại tab `Patch Python`, mình thấy có đoạn xử lý dữ liệu trong list `data` nhưng không thấy list `data` được khai báo ở đâu, nên mình xem tab `Original Bytecode` thì thấy có đoạn LOAD hàng loạt byte int rồi lưu vào biến được đặt tên là `data`

    ![alt text](../../images/babyvm05.png)

- Như vậy là mình đã có đủ các dữ kiện để tiến hành phân tích.

## [2] PHÂN TÍCH
- Tên đề bài là "VM", từ này có lẽ là viết tắt của "Virtual Machine". Hay nói cách khác đây chính là 1 kiến trúc máy tính tự custom.
- Dưới đây là mô tả về các kiến trúc tập lệnh của máy ảo này:
    - Push/Pop & Load
        - `161` val: push immediate value val vào stack.
        - `162`: push reg1 vào stack.
        - `163`: push reg2 vào stack.
        - `177`: pop stack → lưu vào reg1.
        - `178`: pop stack → lưu vào reg2.
    - Arithmetic
        - `81:` reg1 += stack.pop()
        - `82:` reg2 += stack.pop()
        - `97:` reg1 -= stack.pop()
        ...
        - `130`: reg2 *= stack.pop()
    - Comparison
        - `144`: nếu reg1 != reg2 → check = False
    - No-op
        - `105`: chỉ tăng IP.
- Ở đầu chương trình có đoạn kiểm tra và crack lấy key, chính là mã hash `da985b099592755e04c6be1188e42c8a2c1d7a7e1783076a8127b79933cee55f`, sử dụng tool crack hash thì sẽ có được key là `virtualmachine`.
- Key này sẽ được xor với list `data` để ra được toàn bộ các byte code tương ứng của tập lệnh trong máy ảo đã phân tích phía trên.
- Tiếp đó phân tích chương trình có thể rút ra 53 phương trình liên quan đến từng ký tự của flag. Từ đó tiến hành sử dụng thư viện z3 trong python để giải hệ phương trình tìm flag.

## [3] SOLVE
- Dưới đây là cách giải của mình cho challenge này:
    ```
    from z3 import *
    ...
    data = [(b ^ KEY[i % len(KEY)]) for i, b in enumerate(data_enc)]

    # ---------- VM opcode map (sau giải mã) ----------
    OP_PUSH_IMM = 161  # push next byte
    OP_PUSH_R1  = 162  # push reg1
    OP_PUSH_R2  = 163  # push reg2
    OP_POP_R1   = 177  # reg1 = pop()
    OP_POP_R2   = 178  # reg2 = pop()
    OP_R1_ADD   = 81   # reg1 += pop()
    OP_R2_ADD   = 82   # reg2 += pop()
    OP_R1_SUB   = 97   # reg1 -= pop()
    OP_R2_SUB   = 98   # reg2 -= pop()
    OP_R1_XOR   = 113  # reg1 ^= pop()
    OP_R2_XOR   = 114  # reg2 ^= pop()
    OP_R1_MUL   = 129  # reg1 *= pop()
    OP_R2_MUL   = 130  # reg2 *= pop()
    OP_CMP_NE   = 144  # if reg1 != reg2: check=False
    OP_NOP      = 105  # ip += 1

    # ---------- Build symbolic flag ----------
    bv = lambda x: BitVecVal(x, BVW)

    flag = [BitVec(f"f{i}", BVW) for i in range(FLAG_LEN)]
    s = Solver()
    ...
    res = s.check()
    if res != sat:
        raise RuntimeError(f"UNSAT — kiểm tra lại key/opcodes. Z3 says: {res}")

    m = s.model()
    flag_str = "".join(chr(m.evaluate(ch).as_long()) for ch in flag)
    print(flag_str)
