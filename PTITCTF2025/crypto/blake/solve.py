import sys

# Tắt giới hạn chuyển đổi int<->str để xử lý các số lớn
sys.set_int_max_str_digits(0)

# Các hằng số từ file chal.py
N = int("", 16)
C = int("", 16)
E = 1234567891
NBIT = 256

def magicc(n: int) -> int:
    """Hàm magicc được định nghĩa trong file chal.py."""
    # Đảm bảo n có đúng NBIT bit để bin(n) không bị thiếu bit 0 ở đầu
    B = bin(n)[2:].zfill(NBIT)
    M = ''.join('01' if b == '0' else '11' for b in B)
    return int(M, 2)

def invert_magicc_str(magic_bin_str: str) -> str:
    """Hàm đảo ngược của magicc, hoạt động trên chuỗi nhị phân."""
    if len(magic_bin_str) % 2 != 0:
        raise ValueError("Chuỗi nhị phân đầu vào phải có độ dài chẵn")
    
    original_bin = ""
    for i in range(0, len(magic_bin_str), 2):
        pair = magic_bin_str[i:i+2]
        if pair == '01':
            original_bin += '0'
        elif pair == '11':
            original_bin += '1'
        else:
            raise ValueError(f"Cặp bit không hợp lệ '{pair}' trong chuỗi magicc")
    return original_bin

print("[+] Bắt đầu quá trình phân tích N...")

# Bước 1: Khôi phục 128 bit cuối của p (p_lo)
k = 2**NBIT
r_lsbs = (-N) % k
print(f"[1] Tính toán 256 bit cuối của r (r_lsbs): {r_lsbs:x}")

# Chuyển r_lsbs sang chuỗi nhị phân 256-bit
# SỬA LỖI: zfill(NBIT) thay vì zfill(NBIT * 2)
# magicc(p_lo) với p_lo 128-bit sẽ tạo ra một số 256-bit.
r_lsbs_bin = bin(r_lsbs)[2:].zfill(NBIT)

# Đảo ngược magicc để tìm 128 bit cuối của p
p_lo_bin = invert_magicc_str(r_lsbs_bin)
p_lo = int(p_lo_bin, 2)
print(f"[2] Khôi phục 128 bit cuối của p (p_lo): {p_lo:x}")

# Bước 2: Dùng tìm kiếm nhị phân để tìm 128 bit đầu của p (p_hi)
target = N >> 640

low = 2**(NBIT // 2 - 1)
high = 2**(NBIT // 2) - 1
p_hi_candidate = -1

print("[3] Bắt đầu tìm kiếm nhị phân cho p_hi...")
while low <= high:
    mid = (low + high) // 2
    if mid == 0: # Tránh lỗi magicc(0)
        low = mid + 1
        continue
    # Để magicc hoạt động đúng, cần zfill cho mid
    mid_magicc = int(''.join('01' if b == '0' else '11' for b in bin(mid)[2:].zfill(NBIT//2)), 2)
    approx_val = mid * mid_magicc
    
    if approx_val < target:
        low = mid + 1
    elif approx_val > target:
        high = mid - 1
    else:
        p_hi_candidate = mid
        break

# Kết quả tìm kiếm nhị phân sẽ rất gần với giá trị đúng
# Ta sẽ kiểm tra một vài giá trị xung quanh `low` để xử lý sai số
p_hi = -1
for candidate in range(low - 10, low + 10):
    if candidate <= 0: continue
    p_candidate = (candidate << (NBIT // 2)) + p_lo
    q_candidate = p_candidate * k + (k - 1)
    if N % q_candidate == 0:
        p = p_candidate
        q = q_candidate
        r = N // q
        p_hi = candidate
        print(f"[+] Tìm thấy p_hi thành công: {p_hi:x}")
        break

if p_hi == -1:
    print("[-] Không tìm thấy p_hi. Thử lại hoặc điều chỉnh vùng tìm kiếm.")
    exit()

print(f"[4] Khôi phục thành công các giá trị:")
print(f"  p = {p:x}")
print(f"  q = {q:x}")
print(f"  r = {r:x}")

# Bước 3: Giải mã RSA
print("\n[+] Bắt đầu giải mã RSA...")
phi = (q - 1) * (r - 1)
print(f"[5] Tính toán phi (Euler's totient): {phi:x}")

d = pow(E, -1, phi)
print(f"[6] Tính toán d (khóa riêng): {d:x}")

m = pow(C, d, N)
print(f"[7] Giải mã được m: {m:x}")

# Chuyển đổi m sang bytes, sau đó sang chuỗi utf-8
byte_length = (m.bit_length() + 7) // 8
m_bytes = m.to_bytes(byte_length, 'big')
required_input = m_bytes.decode('utf-8', errors='ignore')

print("\n" + "="*50)
print("  KẾT QUẢ CUỐI CÙNG")
print("="*50)
print(f"Chuỗi đầu vào cần nhập để lấy FLAG là:\n\n{required_input}")
print("="*50)