...
# -*- coding: utf-8 -*-
"""
Solver cho challenge 's_ij = u_i^T F v_j' (Vandermonde).
- Hỏi 25 s_ij => thu S = V F V^T
- Tính F = V^{-1} S (V^{-1})^T mod P
- Giải mã FLAG từ các khối big-endian
Hỗ trợ:
  * TCP mode: --host HOST --port PORT (kết nối service từ đề)
  * Local mode: --local path/to/chal.py (spawn script gốc)
"""
...
def decode_flag_from_F(F):
    ...
    # bỏ các khối 0 ở CUỐI (do overrun)
    while parts and parts[-1] == 0:
        parts.pop()
    if not parts:
        return b""
    CH = max(needed_bytes(x) for x in parts)  # tối thiểu thỏa mãn mọi khối
    last_len = needed_bytes(parts[-1])        # khối cuối có thể ngắn hơn
    blobs = []
    for x in parts[:-1]:
...
def gather_S_via_local(chal_path):
    ...
    S = [[0]*N for _ in range(N)]
    # ăn banner vài dòng (không bắt buộc)
    try:
        for _ in range(5):
