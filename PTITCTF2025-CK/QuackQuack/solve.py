...
# ----- các hàm/constant giống hệt server -----
BITS = 20
MOD  = 1 << BITS
ROUND_CONST = 0x9E377
...
# --- Python's stdlib random: cần import tại chỗ để tránh shadow ---
import random, time
...
INV_31337 = modinv(31337, 1<<16)  # luôn tồn tại vì 31337 lẻ

def recover_seed_from_commit_ticket(commit_hex: str, ticket: int):
    # giải (seed*31337 + 1337) ≡ ticket (mod 2^16)
    base_x = ((ticket - 1337) * INV_31337) & 0xFFFF
    # 16 ứng viên cho 20-bit seed
    for k in range(16):
        s = base_x + (k << 16)
...
def solve(host="103.197.184.48", port=31337, track_len=60):
    context.log_level = "error"  # gọn log pwntools
    io = remote(host, port)
    ...
    round_idx = 0
    while True:
        # lấy block có COMMIT/TICKET/prompt
        ...
        # mô phỏng để lấy winner
        eng = DuckEngine(seed, n, track_len)
        # “khung chờ” 6 lần render trước đua ở server không ảnh hưởng logic,
        # ta chỉ cần chạy đến khi có vịt cán đích.
        ...
        round_idx += 1
        # đọc kết quả round (quán quân, reveal seed, …)
        post = recv_until_contains("SHA1(seed)", timeout=5.0)
        # nếu đoán sai server sẽ đóng luôn
        if "Bạn đoán sai" in post:
            print(f"[x] Sai ở round {round_idx}")
            return
        # nếu còn round tiếp, server sẽ in 'Đúng! Vào round tiếp theo...'
        if "FLAG:" in post:
            # trường hợp server in flag luôn (hiếm), in ra:
            print(post)
            break
