...
def solve_from_s_state():
    bs = BoolSolver(threads=16)
    ...
    start = time.time()
    sat, model = bs.solver.solve()
    print(f"vars={bs.top} sat={sat} elapsed={time.time() - start:.2f}s")
    if not sat:
        return None
...
if __name__ == "__main__":
    ...
    print("Attempting SAT recovery of the 64-bit S state...")
    solve_from_s_state()
from pycryptosat import Solver
import argparse
import time
