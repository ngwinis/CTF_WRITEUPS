import os, sys
from typing import List, Optional
from flag import flag

# fmt: off
SBOX = [
    ,
]
INV_SBOX = [
    ,
]
# fmt: on


def bytes_to_state(block: bytes) -> List[List[int]]:
    if len(block) != 16:
        raise ValueError("block must be 16 bytes")
    # AES state is column-major
    return [[block[r + 4 * c] for c in range(4)] for r in range(4)]


def state_to_bytes(state: List[List[int]]) -> bytes:
    return bytes(state[r][c] for c in range(4) for r in range(4))


def sub_bytes(state: List[List[int]]) -> None:
    for r in range(4):
        for c in range(4):
            state[r][c] = SBOX[state[r][c]]


def shift_rows(state: List[List[int]]) -> None:
    for r in range(1, 4):
        state[r] = state[r][r:] + state[r][:r]


def xtime(x: int) -> int:
    return (((x << 1) ^ 0x1B) & 0xFF) if (x & 0x80) else ((x << 1) & 0xFF)


def gf_mul(x: int, y: int) -> int:
    z = 0
    while y:
        if y & 1:
            z ^= x
        x = xtime(x)
        y >>= 1
    return z


def mix_columns(state: List[List[int]]) -> None:
    for c in range(4):
        a0, a1, a2, a3 = (state[0][c], state[1][c], state[2][c], state[3][c])
        state[0][c] = gf_mul(a0, 2) ^ gf_mul(a1, 3) ^ a2 ^ a3
        state[1][c] = a0 ^ gf_mul(a1, 2) ^ gf_mul(a2, 3) ^ a3
        state[2][c] = a0 ^ a1 ^ gf_mul(a2, 2) ^ gf_mul(a3, 3)
        state[3][c] = gf_mul(a0, 3) ^ a1 ^ a2 ^ gf_mul(a3, 2)


def inv_sub_bytes(state):
    for r in range(4):
        for c in range(4):
            state[r][c] = INV_SBOX[state[r][c]]


def inv_shift_rows(state):
    for r in range(1, 4):
        state[r] = state[r][-r:] + state[r][:-r]


def inv_mix_columns(state):
    for c in range(4):
        a0, a1, a2, a3 = state[0][c], state[1][c], state[2][c], state[3][c]
        state[0][c] = gf_mul(a0, 14) ^ gf_mul(a1, 11) ^ gf_mul(a2, 13) ^ gf_mul(a3, 9)
        state[1][c] = gf_mul(a0, 9) ^ gf_mul(a1, 14) ^ gf_mul(a2, 11) ^ gf_mul(a3, 13)
        state[2][c] = gf_mul(a0, 13) ^ gf_mul(a1, 9) ^ gf_mul(a2, 14) ^ gf_mul(a3, 11)
        state[3][c] = gf_mul(a0, 11) ^ gf_mul(a1, 13) ^ gf_mul(a2, 9) ^ gf_mul(a3, 14)


def add_round_key(state: List[List[int]], round_key: bytes) -> None:
    if len(round_key) != 16:
        raise ValueError("round key must be 16 bytes")
    for c in range(4):
        for r in range(4):
            state[r][c] ^= round_key[r + 4 * c]


def generate_random_round_keys() -> List[bytes]:
    # K0, K1, K2, K3, K4, K5
    return [os.urandom(16) for _ in range(6)]


def aes5_encrypt_block(plaintext: bytes, round_keys: List[bytes]) -> bytes:
    if len(plaintext) != 16:
        raise ValueError("plaintext must be 16 bytes")
    if len(round_keys) != 6:
        raise ValueError("need exactly 6 round keys")
    if any(len(k) != 16 for k in round_keys):
        raise ValueError("every round key must be 16 bytes")

    state = bytes_to_state(plaintext)

    # initial whitening / AddRoundKey
    add_round_key(state, round_keys[0])

    # rounds 1..4
    for r in range(1, 5):
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        add_round_key(state, round_keys[r])

    # round 5: no MixColumns
    sub_bytes(state)
    shift_rows(state)
    add_round_key(state, round_keys[5])

    return state_to_bytes(state)


def aes5_decrypt_block(ciphertext: bytes, round_keys: list[bytes]) -> bytes:
    if len(ciphertext) != 16:
        raise ValueError("ciphertext must be 16 bytes")
    if len(round_keys) != 6:
        raise ValueError("need exactly 6 round keys")
    if any(len(k) != 16 for k in round_keys):
        raise ValueError("every round key must be 16 bytes")

    state = bytes_to_state(ciphertext)

    # inverse of final round:
    add_round_key(state, round_keys[5])
    inv_shift_rows(state)
    inv_sub_bytes(state)

    # inverse of rounds 4..1
    for r in range(4, 0, -1):
        add_round_key(state, round_keys[r])
        inv_mix_columns(state)
        inv_shift_rows(state)
        inv_sub_bytes(state)

    # inverse of initial AddRoundKey
    add_round_key(state, round_keys[0])

    return state_to_bytes(state)


def read_hex_block(prompt: str, label: str) -> Optional[bytes]:
    s = input(prompt).strip()
    try:
        data = bytes.fromhex(s)
    except ValueError:
        print("invalid hex")
        return None
    if len(data) != 16:
        print(f"{label} must be exactly 16 bytes")
        return None
    return data


def main():
    round_keys = generate_random_round_keys()
    max_queries = 4096
    pt = os.urandom(16)
    assert aes5_decrypt_block(aes5_encrypt_block(pt, round_keys), round_keys) == pt

    queries_used = 0

    def query_or_none(prompt: str, label: str) -> Optional[bytes]:
        nonlocal queries_used
        if queries_used >= max_queries:
            print("query limit exceeded")
            return None
        block = read_hex_block(prompt, label)
        if block is not None:
            queries_used += 1
        return block

    while True:
        try:
            choice = input("> ").strip()
        except EOFError:
            print()
            return

        match choice:
            case "1":
                if (pt := query_or_none("pt> ", "plaintext")) is not None:
                    print(aes5_encrypt_block(pt, round_keys).hex())

            case "2":
                if (ct := query_or_none("ct> ", "ciphertext")) is not None:
                    print(aes5_decrypt_block(ct, round_keys).hex())

            case "3":
                print("submit 6 round keys as hex, one per line")
                guessed = []
                for i in range(6):
                    if (k := read_hex_block(f"k{i}> ", "each round key")) is None:
                        break
                    guessed.append(k)
                else:
                    print(flag if guessed == round_keys else "wrong")
                    if guessed == round_keys:
                        return

            case "4":
                print("bye")
                return

            case _:
                print("invalid option")


if __name__ == "__main__":
    main()
